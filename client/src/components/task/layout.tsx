...
export  function CreateTaskLayout({ children }: { children: React.ReactNode }) {
    ...
    transition={{duration: 0.3}}
      layout
      className="h-[100dvh] overflow-y-auto border-l border-gray-200 fixed top-0 right-0 z-50 bg-white"
  >
...
function BackGroundBlur({onClose}:{onClose:()=>void}) {
    return <div className="fixed inset-0 bg-black/10 z-40 backdrop-blur-sm" onClick={onClose}></div>
}